...
function grundrechte($selected = null) {
    global $tpl;
    $liste = '';
    $res = db_query('SELECT * FROM prefix_grundrechte');
    while ($row = db_fetch_assoc($res)) {
        $sel = '';
        if ($row['id'] == $selected) {
            $sel = '" selected="selected"';
            $liste .= $tpl->list_get('drecht', array($row['id'] . $sel, $row['name']));
        } else {
            $liste .= $tpl->list_get('drecht', array($row['id'], $row['name']));
        }
    }
...
function archiv_downs_admin_showcats($id, $stufe) {
    $q = "SELECT id,name,pos,cat,img FROM prefix_downcats WHERE cat = " . $id . " ORDER BY pos";
    $erg = db_query($q);
    if (db_num_rows($erg) > 0) {
    ...
    case 'downloads' :
        ...
        if (!empty($_POST['sub'])) {
            ...
            if (empty($_POST['pkey'])) {
                ...
                db_query("INSERT INTO prefix_downloads (`time`,`cat`,`creater`,`version`,`url`,surl,`ssurl`,`name`,`desc`,`descl`,pos,`demo`,`drecht`) VALUES (NOW(),'" . $_POST['cat'] . "','" . $_POST['creater'] . "','" . $_POST['version'] . "','" . $_POST['url'] . "','" . $_POST['surl'] . "','" . $_POST['ssurl'] . "','" . $_POST['name'] . "','" . $_POST['desc'] . "','" . $_POST['descl'] . "','" . $pos . "','" . $_POST['demo'] . "','" . $_POST['drecht'] . "')");
            } else {
                ...
                db_query("UPDATE prefix_downloads SET " . $datum . "pos = " . $pos . ", `cat` = '" . $_POST['cat'] . "',`creater` = '" . $_POST['creater'] . "',version = '" . $_POST['version'] . "',url = '" . $_POST['url'] . "',surl = '" . $_POST['surl'] . "',ssurl = '" . $_POST['ssurl'] . "',`name` = '" . $_POST['name'] . "',`desc` = '" . $_POST['desc'] . "',descl = '" . $_POST['descl'] . "',demo = '" . $_POST['demo'] . "',drecht = '" . $_POST['drecht'] . "'  WHERE id = '" . $_POST['pkey'] . "'");
                if ($alt_row['cat'] != $_POST['cat']) {
        ...
        if (isset($_POST['Csub'])) {
            ...
            if (empty($_POST['Cpkey'])) {
                ...
                db_query("INSERT INTO prefix_downcats (`cat`,`name`,`desc`,pos,recht,img) VALUES (" . $_POST['Ccat'] . ",'" . $_POST['Cname'] . "','" . $_POST['Cdesc'] . "','" . $pos . "','" . $_POST['Crecht'] . "','" . $upload->file() . "')");
            } else {
                ...
                if ($bool) {
                    ...
                    db_query("UPDATE prefix_downcats SET `cat` = '" . $_POST['Ccat'] . "',`name` = '" . $_POST['Cname'] . "',pos = '" . $pos . "',`desc` = '" . $_POST['Cdesc'] . "', recht = '" . $_POST['Crecht'] . "', img='" . $upload->file() . "' WHERE `id` = '" . $_POST['Cpkey'] . "'");
                    if ($alt_row['cat'] != $_POST['Ccat']) {
        ...
        // downs
        if ($menu->getA(2) == 'e') {
            $erg = db_query("SELECT id,`cat`,creater,surl,ssurl,pos,version,url,`name`,`desc`,descl,demo,drecht FROM prefix_downloads WHERE id = '" . $menu->getE(2) . "'");
            ...
            $_ilch['drecht'] = grundrechte($_ilch['drecht']);
        } else {
            if (isset($azk)) {
                $c = $azk;
